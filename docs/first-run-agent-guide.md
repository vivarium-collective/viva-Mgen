# mgen + vivarium-workbench — first-run agent guide

> **You are a coding agent (Claude Code / Cursor / …). A first-time user handed
> you this file to get them onto the `mgen` + vivarium-workbench
...
- **This workspace (`mgen`)** — a
  ...
  **typed, independently-wireable processes** you *compose* into whatever
  ...
  `viva_mgen/` (`core.py` exposes `build_core()`); research state lives
...
```bash
git clone <this repo's URL> mgen
cd mgen
```

**Gate:** `workspace.yaml` and `viva_mgen/core.py` exist.
...
**Gate:** `.venv/bin/python -c "import viva_mgen; print('ok')"` prints
...
- **Registry** — the process and type definitions this workspace registers
  (from `viva_mgen/core.py`'s `build_core()`).
...
- **`ImportError` for `viva_mgen` in the dashboard** — the editable
  install didn't run or ran into the wrong venv. Re-run `uv pip install -e .`
  and confirm `.venv/bin/python -c "import viva_mgen"`.
