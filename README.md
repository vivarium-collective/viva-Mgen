...
> ### 🔬 Interactive read-only dashboard
>
> Run `vivarium-workbench serve --workspace .` to explore every composite, study, and
> result in your browser. Once this workspace is pushed to GitHub, the `publish-dashboard`
> workflow publishes a live read-only version to GitHub Pages and this link fills in.
<!-- END:dashboard -->
...
`workspace/investigations/mgen/` binds seven studies, one per figure:

| Study | Figure | What it shows |
|---|---|---|
| `fig1-architecture` | Fig 1 | the integrated cell as composed pbg processes over shared stores |
| `fig2-growth` | Fig 2 | ~9 h doubling, dry-mass composition, growth curve |
| `fig3-expression` | Fig 3 | bursty low-copy mRNA, accumulating protein |
| `fig4-cell-cycle` | Fig 4 | emergent inverse initiation↔replication duration control |
| `fig5-energy` | Fig 5 | ATP/GTP production and expression energy budget |
...
Run a study locally:
    ...
    python workspace/studies/fig2-growth/sims/run.py
